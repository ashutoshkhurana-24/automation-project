...
**`gpt-transcribe` looks like a strict upgrade and is unusable here — and the reason is not the one it first appears to be.** Newer than `gpt-4o-transcribe` and cheaper ($0.0045 against $0.006 a minute), and it returns Devanagari for every clip: 9% word recall against the house's names.

The first reading was that it ignores the romanisation instruction. It is worse and simpler than that: **it ignores the `prompt` parameter entirely.** Proven by asking it the same clip with no prompt, with a nonsense prompt ("use the word ZORBLAX for every noun"), and with a script instruction — **byte-identical output all three times**, where `gpt-4o-transcribe` visibly changes with each. Its model page advertises "unstructured context, keyword hints, and multiple language hints", so all of those were tried on the file endpoint: `prompt`, `keywords`, `context` and `language`/`languages` in every shape, plus six wordings of the instruction and a Whisper-style prompt written *in* romanised Hinglish to bias by continuation. `languages` is at least real — it validates, rejecting an array or a comma list — and none of them moves the script. It transcribes Hindi speech in Devanagari as a property of the model, not as a prompt that could be worded better. Do not retry this.

**And the corollary, which matters more: `gpt-4o-transcribe`'s romanisation depends on the whole prompt.** Given only a short instruction it answered `AC का फैन चालू करो।` — Devanagari, and a worse transcript. The long vocabulary hint is what produces Latin, so **trimming that prompt would silently break the free grammar path**, since a Devanagari transcript matches no room slug. `gpt-4o-mini-transcribe` is half the price and twice as fast but drops to Devanagari on one clip in six and 82% recall. `gpt-4o-transcribe` stays: **98% recall, five of six exact, no Devanagari.** The lesson is the one this file keeps recording — a newer model is not a better one for the job, and the job here is romanised Latin rather than a good transcript.
