...
- **Sleep timers** — `POST /api/timers {minutes, scope}` where scope is `house`, `room:NAME` or `device:ID`; fires through the same `sendSteps()` as a cue. In memory only: a restart drops them, which is the safe direction (a forgotten timer switching the house off hours later is worse than one that quietly lapses).
