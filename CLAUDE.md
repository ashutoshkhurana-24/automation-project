...
The photograph in use is a cold Patagonian landscape — snow, slate, a turquoise lake — which is the opposite of the warm interior first assumed, and better for it. The design's whole claim is that *the only colour on the page is the light a lamp is making*; a cold backdrop makes every amber glow the only warm thing on screen, so the thesis reads louder rather than being muddied by warm-on-warm. Its bright sky and snow sit exactly where the header does, so the top of the scrim is held down hardest (.74) and eases through nine stops to avoid a visible band.
