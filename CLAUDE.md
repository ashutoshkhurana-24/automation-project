...
- **The hub *is* the always-on box.** It is the Dell-branded NAS at `192.168.1.3` — the only device in the house that is always on and always on the LAN. So "where do we host a scheduler" and "the box we're locked out of over SSH" are the *same machine*. But its own scheduler is dead (see the `addScheduledTrigger` bug above), so hosting on it still requires OS access to run *our* scheduler.
